 ...
 * - PUT /api/files/[id] : Mise à jour des métadonnées avec validation native TypeScript
 * - DELETE /api/files/[id] : Suppression sécurisée avec vérification des dépendances
 * - CORRECTION MAJEURE : Suppression complète de Zod, validation native TypeScript
 * - CORRECTION : Validation manuelle avec messages d'erreur détaillés
 * - CORRECTION : Types TypeScript stricts selon votre environnement
...
// ✅ Fonction de validation des données de mise à jour native
function validateUpdateData(data: any): {
  ...
  // Validation des champs optionnels string
  const stringFields = [
    "mimeType",
    "path",
    "description",
  ...
  stringFields.forEach((field) => {
    if (data[field] !== undefined) {
      if (data[field] === null || typeof data[field] === "string") {
        validData[field as keyof UpdateFileData] = data[field];
      } else {
  ...
  // Validation des relations (CUID optionnels)
  ...
  relationFields.forEach((field) => {
    if (data[field] !== undefined) {
      if (data[field] === null) {
        validData[field as keyof UpdateFileData] = null;
      } else if (typeof data[field] === "string" && isValidCUID(data[field])) {
        validData[field as keyof UpdateFileData] = data[field];
      } else {
        errors.push(`${field} doit être un CUID valide ou null`);
      }
    }
  });
...
export async function PUT(
  ...
  try {
    ...
    // ✅ Construction données de mise à jour
...
export async function OPTIONS(): Promise<NextResponse> {
  const optionsResponse = {
    ...
    endpoints: {
      ...
      PUT: "Mise à jour des métadonnées d'un fichier avec validation native",
